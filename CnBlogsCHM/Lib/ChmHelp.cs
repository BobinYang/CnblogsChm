...
namespace Chen.Common
{
    ...
    public class ChmHelp
    {
        ...
        //名称友好转换器
        public Dictionary<string, string> dicConvert = new Dictionary<string, string>();
        ...
        private delegate bool CompileLog(string log);
        ...
        private void Create(string path)
        {
            ...
            foreach (string filename in strFileNames)
            {
                ...
                var name = dicConvert.ContainsKey(filename) ? dicConvert[filename] : Path.GetFileNameWithoutExtension(filename);
                fileItem.AppendLine("	<LI> <OBJECT type=\"text/sitemap\">");
                fileItem.AppendLine("		<param name=\"Name\" value=\"{0}\">".FormatString(name));
            ...
            foreach (string dirname in strDirectories)
            {
                ...
                hhcBody.AppendLine("		<param name=\"Name\" value=\"{0}\">".FormatString(Path.GetFileName(dirname)).Remove(28, 3));
