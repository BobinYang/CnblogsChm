...
namespace Chen.DB
{
    ...
    public class DbCommon
    {
        ...
        /// <param name="returnUrl">携带返回目录链接</param>
        /// <param name="tableDesc">携带返回目录链接</param>
        public static void CreateHtml(DataTable dt, bool KeepNull, string Path, string returnUrl = "", string tableDesc = "")
        {
            ...
            code.AppendLine("        <div class=\"styletab\">{0}{1}{2}</div>".FormatString(
                ...
                returnUrl.Length > 0 ? "<a href='{0}' style='float: right;font-size: 12px;font-weight: normal;'>返回目录</a>".FormatString(returnUrl) : string.Empty));
            code.AppendLine("                        </caption>");
